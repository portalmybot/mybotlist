...
const BotsPremiums = () => {
  ...
  const {isLoading, data: botsListPremium} = useQuery('botsListPremium', getListPremiumBots)
  ...
  return (
      <>
        <Grid container spacing={5}>
          ...
          {!isLoading && botsListPremium ?  
            <>
              { 
                botsListPremium.data.map((bot) => {
                  return (
        ...
        <Grid className={classes.gridBot} container>
          <Grid item xs={12}>
            <Link underline='none' component={RouterLink} className={classes.expand} to={'/bots/premium'} color="inherit">
              <ColorButton startIcon={<PlaylistAddIcon />} variant="contained" size="large" color="default" fullWidth>
                VER MÁS BOTS PREMIUMS
              </ColorButton>
            </Link>
          </Grid>
        </Grid>
